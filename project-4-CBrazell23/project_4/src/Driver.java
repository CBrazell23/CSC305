...
class Driver {
  public static void main(String [] args) throws IOException {
	...
	JsonParser parser;
	int typeAndFormat = DriverHelper.determineType(); // 0: UN 1: FS 2: FN 3: UN+Filter
...
//    String filePath = "/Users/colebrazell/Desktop/CSC305/project-2-CBrazell23/project_2/inputs/simple.txt";
    ...
    if(typeAndFormat == 1) {
      ...
      vis.visit(parser, typeAndFormat);
    } else if(typeAndFormat == 3) {
      parser = new NotSimpleParser(data, Logger.getLogger(Driver.class.getName()));
      JsonParser filterParser = new Filter(parser, DriverHelper.getFilter());
      vis.visit(filterParser, typeAndFormat);
    } else{
      parser = new NotSimpleParser(data, Logger.getLogger(Driver.class.getName()));
      vis.visit(parser, typeAndFormat);
    }
